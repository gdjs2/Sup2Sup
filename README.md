...
2. Optionally **Import… → Video container…**. Choose one video stream for playback (or no video) and check the PGS subtitle tracks to import. The picker shows video dimensions and codec, and subtitle language/title metadata. Choose no video to add subtitles while keeping any existing playback video. All new PGS tracks are extracted together in one scan, then loaded one by one from temporary SUP files. Temporary files are removed after loading or cancellation. Non-PGS tracks (such as SRT, ASS, or VobSub) are skipped with a count. Reimporting the same video avoids duplicating its existing PGS tracks. Replacing/removing the video keeps imported subtitle tracks and edits.
3. **Import… → SUP subtitle files…** adds one or more external tracks to the current project. Choose the active track with the **Subtitle track** selector below the preview; switching preserves video playback and position. The crop and review filters stay in place as you switch tracks. Cue offsets and their undo/redo histories remain independent.
4. Set margins in **subtitle pixels**, then choose **Apply project crop**. The crop applies across existing tracks, converts exactly for each track's resolution, and is inherited by later imports. The centered output-size controls also apply across the project. When switching between tracks in older projects with different crops, the visible crop is kept and applied to the other tracks.
5. With video loaded, **Detect project crop…** suggests a centered crop against a standard canvas, including 1920 × 1080 and 3840 × 2160. This is optional and works before importing subtitles. Review the original size and margins before applying. Detection uses video dimensions, not black-bar analysis of decoded frames; a full-size letterboxed video still needs manual margins. **Match video crop…** also applies its result across the project.
6. Review **Problems only**, drag/nudge cues, or use **Fit problems in this track** / **Fit problems in all tracks**. A safe margin of `0` minimizes movement; `20` leaves 20 native subtitle pixels of space. Undo/redo operates on the active track; undoing or redoing a crop change updates the shared crop across tracks.
7. **Save project** keeps the video reference and selected video stream, track list, active subtitle track, shared crop preset, and every track's edits. **Export… → Choose subtitle tracks…** opens a track picker and destination directory field. Only the active track is initially checked; choose any subset or use **Select all** / **Select none**. Every selected track must pass geometry validation first; problems in unselected tracks do not block export. Existing files for selected outputs are refused; choose another directory. **Export… → Active track to file…** lets you choose a filename and optionally save a preservation report. The shortcuts are **Ctrl+E** for the track picker and **Ctrl+Shift+E** for the active track file export.
...
Import progress identifies the current track with a counter such as **(1/5)**. That counter remains visible during file reading, bitmap decoding, and cue checks; extraction reads selected PGS streams together, so its current-stream counter can alternate as packets arrive.

Loading and batch edits run in the background and can be cancelled without applying partial changes. Export stages and validates every selected track before publishing any output. Output filenames derive from track names, with numeric suffixes for duplicates that stay consistent when exporting different subsets. Source files from the entire project remain protected from overwrite. A filesystem failure during publication can leave an incomplete batch; completed files remain available. The movie itself is never cropped or rewritten.
